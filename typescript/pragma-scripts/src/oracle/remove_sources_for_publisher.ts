...
function parseCommandLineArguments(): OptionValues {
  ...
  program
    .name("remove-sources-for-publisher")
    .description(
      "CLI to remove multiple sources for a publisher in PublisherRegistry",
    )
    .requiredOption(
      "--publisher <name>",
      "Name of the publisher to remove the sources from",
    )
    .requiredOption("--sources <names...>", "Names of the sources to remove")
    .requiredOption(
      "--chain <chain_name>",
...
async function main() {
  ...
  console.log(
    `👉 Removing sources for publisher in contract ${publisherRegistryAddress} on chain ${options.chain}`,
  );
  ...
  console.log("✅ Sources removal process completed!");
}
