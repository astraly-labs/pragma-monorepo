...
async function profileGasUsage(
  ...
  allowFailedTransactions = false,
) {
  const receipt = await ensureAccepted(
    await provider.waitForTransaction(transactionHash),
    provider,
  );
  if (!allowFailedTransactions) {
    await ensureSuccess(receipt, provider);
  }
  ...
  if (
    !Object.keys(rawResources).every((key) => expectedResources.includes(key))
  ) {
    throw new Error(
      `unexpected execution resources: ${Object.keys(rawResources).join()}`,
    );
  }
  ...
  const gasPrice = BigInt(
    paidInStrk
      ? blockInfo.l1_gas_price.price_in_fri
      : blockInfo.l1_gas_price.price_in_wei,
  );
  ...
  const gasPerComputationCategory = Object.fromEntries(
    Object.entries(executionResources)
      ...
      .map(([resource, usage]) => [
        resource,
        Math.ceil(usage * gasWeights[resource]),
      ]),
  );
  const maxComputationCategory = maxBy(
    Object.entries(gasPerComputationCategory),
    ([, gas]) => gas,
  )![0];
  const computationGas = BigInt(
    gasPerComputationCategory[maxComputationCategory],
  );
  ...
  if (rawResources.data_availability) {
    ...
    daFee =
      (rawResources.data_availability.l1_gas +
        rawResources.data_availability.l1_data_gas) *
      dataGasPrice;
  ...
  const sortedResources = Object.fromEntries(
    sortBy(Object.entries(executionResources), 0),
  );

  // L2 payloads
  const { calldata, signature } = (await provider.getTransaction(
    receipt.transaction_hash,
  )) as any;
  const calldataGas =
    calldata && signature
      ? Math.floor(
          (calldata.length + signature.length) * l2PayloadsWeights.calldata,
        )
      : undefined; // TODO find workaround for deployment transactions

  const eventGas = Math.floor(
    receipt.events.reduce(
      (sum: any, { keys, data }: { keys: any; data: any }) =>
        sum +
        keys.length * l2PayloadsWeights.eventKey +
        data.length * l2PayloadsWeights.eventData,
      0,
    ),
  );
  return {
...
export function newProfiler(provider: RpcProvider) {
  ...
  return {
    async profile(
      ...
      const profile = await profileGasUsage(
        transactionHash,
        provider,
        allowFailedTransactions,
      );
      if (printProfile) {
    ...
    summarizeCost(profile: Profile) {
      ...
      const feeUsd =
        Number((10000n * profile.actualFee * usdVal) / 10n ** 18n) / 10000;
      return {
        ...
        "Storage diffs": sum(
          profile.storageDiffs.map(
            ({ storage_entries }) => storage_entries.length,
          ),
        ),
    ...
    updateOrCheckReport() {
      ...
      exec(`diff ${filename} ${newFilename}`, (err, stdout) => {
        ...
        if (process.argv.includes("--write")) {
          ...
          console.error(
            `⚠️  Please update ${filename} and commit it in this PR.\n`,
          );
          return process.exit(1);
        } else {
          console.log(
            `Usage: append either --write or --check to the CLI command.`,
          );
        }
      });
    },
