...
function getChainConfig(chainId: string): ChainConfig {
  const chainConfigs: { [key: string]: ChainConfig } = {
    ...
    "0x534e5f4d41494e": { contractId: "0x3333333333333333333333333333333333333333" }, // StarkNet Mainnet
    "0x534e5f474f45524c49": { contractId: "0x4444444444444444444444444444444444444444" }, // StarkNet Sepolia (Goerli)
...
function parseCommandLineArguments() {
  ...
  program
    ...
    .requiredOption('--private-key <private_key>', 'Private key to sign the transaction')
    .option('--theoros-endpoint <url>', 'Theoros endpoint URL', 'https://theoros.pragma.build/')
    .parse(process.argv);
...
async function main() {
  ...
  console.log(`Updating data feed ${feedId} for contract ${chainConfig.contractId} on chain ${options.targetChain}`);
