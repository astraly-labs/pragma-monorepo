...
abstract contract HyperlaneTestUtils is Test {
    ...
    function generateUpdateData(
        ...
        bytes32 domainHash = keccak256(
            abi.encodePacked(emitterChainId, merkleTreeHookAddress, "HYPERLANE")
        );
...
contract HyperlaneTestUtilsTest is Test, HyperlaneTestUtils {
    ...
    function assertHyMsgMatchesTestValues(
        ...
        assertEq(
            ...
            "Timestamp does not correspond"
        );
        assertEq(
            hyMsg.emitterChainId,
            TEST_EMITTER_CHAIN_ID,
            "Emitter chain id does not correspond"
        );
        assertEq(
            hyMsg.emitterAddress,
            TEST_EMITTER_ADDR,
            "Emitter address does not correspond"
        );
        ...
        assertEq(
            ...
            TEST_NUM_SIGNERS,
            "Num signers does not correspond"
        );
    }
    ...
    function testGenerateUpdateDataWorks() public {
        ...
        IHyperlane hyperlane = IHyperlane(
            setUpHyperlane(uint8(validators.length), validators)
        );
        ...
        (HyMsg memory hyMsg, bool valid, string memory reason, , ) = hyperlane
            .parseAndVerifyHyMsg(updateData);
        assertHyMsgMatchesTestValues(
            hyMsg,
            valid,
            reason,
            updateData,
    ...
    function testParseHyMsg() public {
        ...
        assertEq(
            parsedMsg.signatures[0].validatorIndex,
            0,
            "Incorrect validator index for first signature"
        );
        assertEq(
            uint256(parsedMsg.signatures[0].r),
            1,
            "Incorrect r for first signature"
        );
        assertEq(
            uint256(parsedMsg.signatures[0].s),
            2,
            "Incorrect s for first signature"
        );
        assertEq(
            parsedMsg.signatures[0].v,
            27,
            "Incorrect v for first signature"
        );

        // Check second signature
        assertEq(
            parsedMsg.signatures[1].validatorIndex,
            1,
            "Incorrect validator index for second signature"
        );
        assertEq(
            uint256(parsedMsg.signatures[1].r),
            3,
            "Incorrect r for second signature"
        );
        assertEq(
            uint256(parsedMsg.signatures[1].s),
            4,
            "Incorrect s for second signature"
        );
        assertEq(
            parsedMsg.signatures[1].v,
            28,
            "Incorrect v for second signature"
        );
        ...
        assertEq(
            uint256(parsedMsg.emitterAddress),
            6,
            "Incorrect emitter address"
        );
        bytes32 domainHash = keccak256(
            abi.encodePacked(
                parsedMsg.emitterChainId,
                merkleTreeHookAddress,
                "HYPERLANE"
            )
        );
