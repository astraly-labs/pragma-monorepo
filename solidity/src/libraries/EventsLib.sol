...
library EventsLib {
    ...
    event SpotMedianUpdate(bytes32 indexed feedId, uint64 publishTime, SpotMedian spotMedian);
    ...
    event RealizedVolatilityUpdate(bytes32 indexed feedId, uint64 publishTime, RealizedVolatility realizedVolatility);

    /// @dev Emitted when an Options feed with `feedId` has received a fresh update.
    /// @param feedId Pragma Feed ID.
    /// @param publishTime Unix timestamp of the update.
    /// @param options New Options data.
    event OptionsUpdate(bytes32 indexed feedId, uint64 publishTime, Options options);
