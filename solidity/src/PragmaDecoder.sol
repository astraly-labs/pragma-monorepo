...
abstract contract PragmaDecoder {
    ...
    {
        {
            ...
            (hyMsg, valid, reason, index, checkpointRoot) = hyperlane
                .parseAndVerifyHyMsg(encodedHyMsg);
    ...
    ) internal pure returns (uint256 encodedOffset) {
        unchecked {
            encodedOffset = 0;
        ...
        unchecked {
...

            {
...

                {
                    ...
                    numUpdates = UnsafeBytesLib.toUint8(
                        encodedPayload,
                        payloadOffset
                    );
        ...
        )
    {
        unchecked {
            ...
            offset += 2;
            {
                encodedProof = UnsafeCalldataBytesLib.slice(
                    encoded,
                    offset,
                    proofSize
                );
                ...
                encodedUpdate = UnsafeCalldataBytesLib.slice(
                    encoded,
                    encodedUpdateIndex,
                    encoded.length - 40 - encodedUpdateIndex
                ); // 32 bytes for feedId, 8 bytes for publishTime
                fulldataFeed = UnsafeCalldataBytesLib.slice(
                    encoded,
                    encodedUpdateIndex,
                    encoded.length - encodedUpdateIndex
                );
            }
            // bool valid;
        ...
        )
    {
        ...
        feedId = bytes32(
            UnsafeCalldataBytesLib.toUint256(encodedDataFeed, offset)
        );
        offset += 32;
        ...
        unchecked {
            for (uint256 i = 0; i < numUpdates; i++) {
                ...
                (
                    ...
                    feedId,
                    publishTime
                ) = extractDataInfoFromUpdate(
                    updateData,
                    offset,
                    checkpointRoot
                );
                updateLatestDataInfoIfNecessary(
                    feedId,
                    parsedData,
                    publishTime
                );
    ...
    function updateLatestDataInfoIfNecessary(
        ...
        if (parsedData.dataType == FeedType.SpotMedian) {
            if (publishTime > spotMedianFeeds[feedId].metadata.timestamp) {
                ...
                emit EventsLib.SpotMedianUpdate(
                    feedId,
                    publishTime,
                    parsedData.spot
                );
            ...
            if (publishTime > rvFeeds[feedId].metadata.timestamp) {
                ...
                emit EventsLib.RealizedVolatilityUpdate(
                    feedId,
                    publishTime,
                    parsedData.rv
                );
