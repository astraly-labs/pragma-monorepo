...
use crate::types::{aws_credentials::AwsChainCredentialsProvider, CheckpointSyncer, CheckpointWithMessageId};
...
impl S3Storage {
    async fn write_to_bucket(&self, key: String, body: &str) -> Result<()> {
        ...
        timeout(Duration::from_secs(S3_REQUEST_TIMEOUT_SECONDS), self.authenticated_client().put_object(req)).await??;
        Ok(())
    }
...
impl CheckpointSyncer for S3Storage {
    ...
    async fn write_latest_index(&self, index: u32) -> Result<()> {
        ...
        self.write_to_bucket(S3Storage::latest_index_key(), &serialized_index).await?;
        Ok(())
    }
